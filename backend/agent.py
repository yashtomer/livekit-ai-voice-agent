...
_WHISPER_WARMED: set[str] = set()


def _warmup_whisper(size: str, language: str) -> None:
    """Synchronously ping the chosen Whisper container with 1s of silence so
    the model is loaded before the first real transcription. Idempotent —
    only the first call per (size, language) actually does work; later calls
    return immediately. Failures are logged, not raised.
    """
    key = f"{size}:{language}"
    with _WHISPER_WARM_LOCK:
        if key in _WHISPER_WARMED:
            return
    ...
    try:
        ...
        httpx.post(
            f"{WHISPER_URLS[size]}/audio/transcriptions",
        ...
        with _WHISPER_WARM_LOCK:
            _WHISPER_WARMED.add(key)
        logger.info(f"whisper ({size}) warmed for {language} (just-in-time)")
    except Exception as e:
        logger.warning(f"whisper ({size}) JIT warmup failed: {e}")
...
def build_stt(cfg: dict, language: str = "en"):
    ...
    if provider == "whisper_local":
        ...
        # Just-in-time warmup so the first transcription doesn't time out on a
        # cold model load (worst case ~90s on first download/CT2 conversion).
        # Process-init warmup covers most cases; this catches sessions that
        # arrive before the background warmup finishes, or after a container
        # restart.
        _warmup_whisper(size, language)
...
async def entrypoint(ctx: JobContext):
    ...
    # If the model files aren't present (first run, no internet, etc.) we
    # fall back to running without learned turn detection — the call still
    # works, just with simpler endpointing.
    turn_model = None
    try:
        turn_model = EnglishModel() if language == "en" else MultilingualModel()
    except Exception as e:
        msg = str(e)
        logger.error(f"Turn-detector model unavailable: {msg}")
        which = "english" if language == "en" else "multilingual"
        await _send_to_browser(ctx, {
            "type": "model_missing",
            "model": f"turn-detector-{which}",
            "message": (
                f"Turn-detector model ({which}) is not downloaded. "
                "Open Admin → Setup to download it. The call will continue "
                "with simpler turn-taking."
            ),
            "detail": msg,
        })
    ...
    # Capture provider names for context-aware error messages.
    tts_provider = (cfg.get("tts") or {}).get("provider", "")
    stt_provider = (cfg.get("stt") or {}).get("provider", "")
    llm_provider = (cfg.get("llm") or {}).get("provider", "")

    def _tts_alt_hint() -> str:
        """Suggest a TTS *other than* the one currently failing."""
        alts = ["Piper (local)", "Voicebox (local)"]
        if tts_provider != "edge":
            alts.insert(0, "Microsoft Edge TTS (free)")
        # If the failing provider IS local, suggest a cloud one too
        if tts_provider in {"piper_local", "voicebox", "edge"}:
            alts.append("ElevenLabs")
        return " or ".join(alts[:2])
    ...
    def on_session_error(ev):
        ...
        if "terms acceptance" in raw or "model_terms_required" in raw:
            ...
            if tts_provider == "edge" and ("NoAudioReceived" in raw or "no audio" in raw.lower()):
                msg = (
                    "Microsoft Edge TTS returned no audio — the free service throttled this IP "
                    f"or the network blocked it. Switch to {_tts_alt_hint()}."
                )
            elif tts_provider:
                msg = f"Text-to-Speech failed ({tts_provider}). Try switching to {_tts_alt_hint()}."
            else:
                msg = "Text-to-Speech failed. Try switching to a different TTS provider."
        elif ("stt_error" in raw or "STTError" in raw) and "timeout" in raw.lower():
            msg = (
                "Speech-to-Text timed out — the Whisper model is still loading. "
                "Wait ~30 seconds and try again, or pick a smaller size (e.g. tiny)."
            )
        elif "stt_error" in raw or "STTError" in raw:
            alt = "Whisper local (tiny)" if stt_provider != "whisper_local" else "Deepgram or a different Whisper size"
            msg = f"Speech-to-Text failed ({stt_provider or 'unknown'}). Try {alt}."
        elif "llm_error" in raw or "LLMError" in raw:
            alt = "a local Ollama model" if llm_provider != "ollama" else "a smaller Ollama model or a cloud LLM"
            msg = f"Language Model failed ({llm_provider or 'unknown'}). Try switching to {alt}."
...
def _background_warmup():
    ...
    # Pre-warm every Whisper container so the first transcription doesn't pay
    # the model-load cost at session start. Cold first-time load is up to ~90s
    # (download + CT2 conversion); subsequent warm loads are ~5s.
    # If we don't warm these, the first STT call routinely exceeds the 30s
    # request timeout and surfaces as "Speech-to-Text failed".
    import io, wave
    _warmup_buf = io.BytesIO()
    with wave.open(_warmup_buf, "wb") as w:
        w.setnchannels(1)
        w.setsampwidth(2)
        w.setframerate(16000)
        w.writeframes(b"\x00\x00" * 16000)  # 1s of silence
    _warmup_bytes = _warmup_buf.getvalue()

    for _size in ("tiny", "base", "small", "base-multi", "small-multi"):
        _lang = "hi" if _size.endswith("-multi") else "en"
        try:
            httpx.post(
                f"{WHISPER_URLS[_size]}/audio/transcriptions",
                files={"file": ("warmup.wav", _warmup_bytes, "audio/wav")},
                data={
                    "model": WHISPER_MODEL_NAMES[_size],
                    "language": _lang,
                    "response_format": "json",
                },
                timeout=180.0,
            )
            logger.info(f"whisper ({_size}) warmed for {_lang}")
        except Exception as e:
            logger.warning(f"whisper ({_size}) warmup failed: {e}")
